...
VibeGap is a Next.js / TypeScript MVP that helps you **choose places** from a travel or visit plan—not only by star ratings, but by **goal fit**, **review-backed risk**, **confidence**, **distance and landmark context**, and **explainable tradeoffs**.
...
The app uses **Google Places** (Places API New) for place discovery and details, **available review-derived signals** where the API returns them, and **rule-based, goal-weighted scoring** on the server. There is **no** integration with TikTok, Instagram, live social feeds, or scraped social trend data.
...
- **Single-place venue check** — Deep report for one named venue with decision labels and evidence-oriented copy.
...
- **Reliable / Balanced / Discovery** — Local re-ranking styles on the same candidate set (emphasis on established signals vs more exploratory ordering); no extra network calls.
...
- **Tune Ranking priorities** — User-adjustable weights that re-order the shortlist within the same scored pool.
- **Review Evidence** — Surfaces review-backed themes and excerpts when Google returns usable review text.
...
2. **Generate or resolve candidates** — Google Places text search (and related flows) produce or narrow a candidate pool for recommendations; single-place and compare paths resolve named venues.
...
5. **Goal-weighted scoring** — Rule-based fit scores combine intent alignment, review signals, price and operational cues, and geography where applicable.
6. **Recommendation style and tuning** — Optional style (Reliable / Balanced / Discovery) and Tune Ranking sliders apply **local** re-ordering on top of gated scores.
...
- **No live social media or trend data** — positioning and “vibe” language in the app come from query parsing, Places fields, and review-derived signals, not from social scraping.
