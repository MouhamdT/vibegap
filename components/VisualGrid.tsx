...
/** Deterministic text cues used for scoring (not live social feeds). */
export function VisualGrid({ posts }: VisualGridProps) {
  if (posts.length === 0) {
    return (
      <p className="text-[12px] leading-relaxed text-stone-600">
        Representative review snippets are not bundled here. Use Review evidence above for verbatim review-backed
        lines; scoring still uses review themes from Google Places where available.
      </p>
    );
  }
  ...
  return (
    <ul className="space-y-2">
      {posts.map((post) => (
        ...
        >
          <p className="text-[10px] font-medium uppercase tracking-wide text-stone-500">Internal goal cue (not a feed)</p>
