...
/** Deterministic goal signals on cards used for scoring (not live feeds). */
  ...
  if (posts.length === 0) {
    return (
      <p className="text-[12px] leading-relaxed text-stone-600">
        ...
        returns them; scoring still blends those themes with short goal signals on the cards.
      </p>
    );
  }
  ...
  return (
    <ul className="space-y-2">
      {posts.map((post) => (
        ...
        >
          <p className="text-[10px] font-medium uppercase tracking-wide text-stone-500">Goal signal on the card (scoring)</p>
